...
Effect size, interval, forward preservation, and the maximum five-percentage-point increases in failure/inversion/contact all pass. The additional requirement that both arms have **zero prefix-only episodes** alone makes `quality_pass` false for the primary. The identical seed 520025 ends at step 34 in baseline, primary, and every selected control. It has no post-onset outcome and contributes zero to both arms' recorded behavioral mean, retaining a paired difference of zero. This conservative intention-to-treat convention dilutes the effect without inventing post-termination locomotion.
...
The independent audit found that confirmation mean squared action rises from **0.24702 to 0.29108**, while original environment reward falls from **6.23937 to 5.86443** per analyzed step. A post hoc yaw-aligned coordinate calculation gives **+0.05943 m/s** side-velocity change, compared with **+0.40238 m/s** in world Y. This supports interpreting the effect as directional/heading steering while preserving much of the existing gait, without proving a sole mediator. That descriptive analysis did not change fitting, selection or acceptance. Episode averages use available post-onset durations; early-ended episodes do not establish full-horizon persistence.
