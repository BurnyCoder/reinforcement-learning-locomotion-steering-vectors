...
Experiment 001 showed that a few near-stationary recovery windows could dominate activation contrasts despite passing instantaneous health checks. The new hypothesis was that contrasts drawn from sustained running would produce useful speed, effort, or height control. A fitting-only speed floor at half the median healthy fitting-window speed was registered before collecting the new calibration evidence. The fraction is a pilot heuristic motivated by the [001 sensitivity analysis](hc-classic-001-diagnostics.md), not a threshold supplied by an upstream paper.
...
After 74 validation conditions, physical inspection found a separate analysis problem. The speed vector at −0.05 passed the original pooled quality rule despite seed 110001 spending 400 consecutive steps stalled on its head. Nine clean trials diluted the mean contact fraction to **4.47%**. Named simulator contacts and identical replayed arrays ruled out a geometry-indexing or replay mismatch. HalfCheetah's own episode termination does not diagnose this failure: the [official environment documentation](https://gymnasium.farama.org/environments/mujoco/half_cheetah/#episode-end) specifies time-limit truncation and no task termination.

Calibration paused before selection or any confirmation. The registered correction first labels an episode physically failed if its post-onset inversion or forbidden-contact fraction exceeds 5%, combining this with explicit failure or termination. It then averages episode failure labels. Pooled contact/inversion fractions and the existing maximum five-percentage-point increases remain in the gate. The failed speed condition now has a **10% physical-failure rate**, so it fails quality. The 5% episode cutoff is a project heuristic, not a Gymnasium rule.
...
The overall research goal allows the useful effect to differ from the fitting label. Experiment **hc-height-speed-003** will therefore test a new, explicit hypothesis: the frozen **height-derived direction can control forward speed** while retaining competent locomotion. It will reuse the exact 002 height vector and its random/shuffled family, disclose that fitting and diagnostic data come from 002, and calibrate against the speed gate on fresh validation seeds **210000–210009**. Confirmation **220000–220029** and replication **230000–230029** remain fresh and follow a locked choice. The original eight nonzero strengths are retained. This is a new target hypothesis informed by 002, not a retroactive successful score for this experiment. Finer posture calibration at smaller strengths remains an optional later hypothesis.
