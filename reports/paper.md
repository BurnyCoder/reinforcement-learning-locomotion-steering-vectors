...
Both checkpoints are public Farama-Minari releases [R3, R4]. Training seed and step counts were read from their ZIP metadata without executing serialized objects. Saved optimizer settings include learning rate 0.0003, discount 0.99, batch size 256, and replay capacity 1,000,000; these are provenance, not hyperparameters tuned here. Curriculum and full upstream learning curves were not established by this audit. There was **no local policy training, fine-tuning, action-demonstration fitting, or behavioral cloning**. Each environment is represented by one upstream training run; many evaluation resets do not increase that number.
...
Measured targets are forward velocity (m/s), lateral velocity on the ground (m/s), mean torso height (m), yaw rate (rad/s), and an effort proxy: mean squared normalized actions. Lateral is the world's ground-plane y velocity; it does not establish body-frame sidestepping or jumping. The effort proxy is not motor energy, work, or battery consumption. Turning uses quaternion-derived orientation with wrapped angle differences; no quaternion coordinate is treated as an angle. Original reward, action saturation, inversion, inappropriate ground contact and episode failure are recorded alongside target effects [R5, R6].
...
Every primary direction has three random directions and a shuffled-label direction, all matched in norm and given the same validation strength grid. When a primary candidate qualifies, a constant action-bias comparator is fitted using every tenth post-startup source observation. It averages steered-minus-baseline deterministic actions at the selected hidden strength; the largest positive comparator-grid strength reproduces that mean displacement, with action clipping at runtime. This is a fixed action offset, not a learned imitation policy. Exact vectors, control provenance, action biases and source mappings remain saved. In retargeted runs, an alias such as `speed` can denote a height-derived array; the original extraction label remains explicit.

<!-- pagebreak -->

## 3. Evaluation, acceptance and adaptive chronology

Evaluation uses deterministic actions and paired reset seeds. Each baseline/treatment pair must have matching complete simulator-state hashes immediately before intervention. Target effects are treated-minus-baseline differences of episode means. Confidence intervals use 2,000 percentile-bootstrap resamples of paired episodes, not correlated timesteps; bootstrap seeds are 710 for validation and 720 for confirmation. Intervals are nominal, with no correction for the many exploratory conditions.
...
All targets also require an interval excluding zero and quality acceptance. The quality-increase rule permits at most five percentage points additional failure, inversion, or forbidden contact. After the 002 measurement review, an individual episode is physically failed when its post-onset inversion or forbidden-contact fraction exceeds 5%, combined with explicit termination/runtime failure. Pooled time fractions remain separately reported. A further implementation criterion requires zero **prefix-only** episodes in both arms: any episode ending before intervention disqualifies the full condition. Such pairs remain in the denominator with zero post-onset behavioral contribution, rather than invented motion after termination. This last criterion determines Ant 005's formal outcome and must be distinguished from intervention-induced failure.
...
### 4.1 Rejected extraction and the pooled-quality problem
...
After 74 validation conditions, inspection of speed −0.05 found that **4.47% pooled contact concealed one episode stalled on its head for 400 consecutive steps**, or 20 simulated seconds. Named geometry contacts and identical replayed arrays ruled out an indexing/replay mismatch. Quality analysis was amended to classify failure per episode before averaging, while retaining the existing five-point allowance. The same condition then had one failure in ten trials and was rejected. All 120 final conditions were analyzed consistently; the old 74 estimates retain identical target effects, intervals and seed metadata. Two gate decisions changed. This documented amendment used validation observations, not held-out results.
...
The full validation set contains 88 conditions: 80 activation settings across lateral/turning families plus eight action-bias settings. Eleven conditions pass pilot gates, including two primary lateral settings and nine generic controls. The stronger eligible primary response at −0.1 was locked, together with five controls, before confirmation at 00:09:03 UTC. Selecting a direction from this exploratory grid requires the separate fresh test; the figure alone cannot establish reliable usefulness.

<!-- pagebreak -->

## 6. Ant held-out effect and the conservative prefix criterion
...
HalfCheetah checkpoint revision: `b4ce04da6f246f06ae4c4258b8ab39624e6600b4`; Ant: `de5978d34b0118341df2b0a30232c5d2bfcb148e`. Archive and parameter SHA256 values are in each manifest. Recorded commits include 001 `6fb6381`, 003 `6600176` and 004 `2377452`. Ant 005 diagnosis/fitting used `8ad6ce3`; its validation/confirmation invocation records `1b835f98446b86aa30c8ba3e50a6826b12ef4068` in the timestamped phase log, with source hashes and a documented original-grid manifest migration. Run 002 lacks a manifest code-commit field and instead retains source hashes and a loader audit. These phase distinctions and that provenance gap are disclosed.

During 002, an inference-only loader override reduced an unused replay-buffer allocation from 308,000,000 to 1,540 bytes per model. Policy hashes and every saved array matched in three audited episodes before cached runtime identities were migrated. This was not a training-resumption equivalence test. Raw scientific outcomes were unchanged. Non-executable NPZ arrays are loaded with `allow_pickle=False`; full trajectories, source vectors, old analyses and immutable selections remain available for audit.
...
[R4] Farama-Minari. **Ant-v5 SAC medium checkpoint**. [Pinned model files](https://huggingface.co/farama-minari/Ant-v5-SAC-medium/tree/de5978d34b0118341df2b0a30232c5d2bfcb148e). One saved upstream seed per policy, with no new policy training in this study.
...
[R7] **RL locomotion steering vectors: implementation and experiment records**. [Source repository](https://github.com/BurnyCoder/rl-locomotion-steering-vectors). [Detailed methods](../docs/methodology.md), [source/reuse audit](../docs/sources.md), and [cumulative experiment register](experiments.md). This is the project evidence, not independent validation.
...
[E1] **hc-classic-001**. [Fitting rejection and sensitivity analysis](hc-classic-001-diagnostics.md); [complete compact bundle](hc-classic-001/README.md).

[E2] **hc-running-002**. [Results and measurement amendment](hc-running-002-findings.md); [physical contact inspection](hc-running-002-validation-inspection.md); [complete compact bundle](hc-running-002/README.md), including the archived 74-condition analysis and comparison audit.

[E3] **hc-height-speed-003**. [Failed coarse-grid retargeting](hc-height-speed-003-findings.md); [complete compact bundle](hc-height-speed-003/README.md), including unchanged-vector audit.

[E4] **hc-height-speed-004**. [Pre-registration](hc-height-speed-004-preregistration.md); [failed speed confirmation](hc-height-speed-004-findings.md); [complete compact bundle](hc-height-speed-004/README.md).

[E5] **ant-classic-005**. [Pre-registration](ant-classic-005-preregistration.md); [fitting inspection](ant-classic-005-diagnostics.md); [lateral results and prefix-only criterion](ant-classic-005-findings.md); [complete compact bundle](ant-classic-005/README.md). Preliminary paired media and ground-plane plot use the first three fixed validation seeds.

Earlier numerical uploads are preserved at [Hugging Face 002 commit 9fa4bb9](https://huggingface.co/BurnyCoder/rl-locomotion-steering-vectors/commit/9fa4bb97ef2ad7cc50d2888146c68407bc850d7c) and [003 raw-run commit 238bd2d](https://huggingface.co/BurnyCoder/rl-locomotion-steering-vectors/commit/238bd2d9eba45fc1e69588f2beb2bbf15450f7b9). Later document/media publication is separate from these numerical outcomes; local bundles identify their own contents and current provenance.
