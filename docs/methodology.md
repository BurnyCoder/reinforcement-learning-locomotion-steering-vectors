...
The review paused calibration after 74 conditions. Those original derived estimates remain in `runs/hc-running-002/validation_pooled-quality-v1.json`; cached raw rollouts, vectors, and simulator measurements were not changed. All validation conditions are recomputed using the revised derived failure criterion before selection and continuation. This is an explicitly documented measurement revision informed by exploratory validation, not a claim that the original calibration protocol was untouched. No held-out evidence informed the revision, and all failed episodes remain in statistical denominators.
