...
The documented pinned-vector download and `replay` command were run from the published bundle: all 18 resulting arrays/metadata fields matched the original steered validation episode byte for byte. GitHub's Markdown renderer returned three actual video players for the README attachments. The active core suite passed 81 tests; paused PGS code was excluded because it is not part of these experiments or this publication.
